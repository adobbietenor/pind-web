...
- **Move the custom auth domain to production** (Alex, M3.1). `auth.pind.social` went onto **pind-staging in M3.1** ($10/month plus Pro), because staging *is* the public product until this milestone and the `supabase.co` sign-in sheet looked like a scam page. **A Supabase project holds one custom domain**, so here it is removed from staging and added to `pind-prod`: the DNS records re-verified, then **about an hour of provider updates** — Google Cloud's authorised redirect URI, the Apple Services ID's domain and return URL (a fresh client secret is minted anyway, for the re-pointed Services ID above), and the app's and Worker's Supabase URL, with a rebuild. Staging falls back to its `supabase.co` URL, which never stopped working.
