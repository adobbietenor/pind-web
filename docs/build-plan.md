...
- **A custom auth domain** (`auth.pind.social`) — **not on staging; decided here** (Alex, M3.1). Cost, as Alex priced it: **Pro at $25/month plus the $10/month add-on per project — about $70/month once production exists too**, for something cosmetic. **It starts mattering the day strangers arrive from Reddit, not before**: the Google sheet then says "continue to mxuajvlrkggrqpntekqt.supabase.co", which reads like a phishing page. A project holds one custom domain. Doing it: the add-on on `pind-prod`, `supabase domains create` → a **DNS-only (grey cloud)** CNAME `auth` → the project host and the `_acme-challenge.auth` TXT it prints → `domains reverify` → add `https://auth.pind.social/auth/v1/callback` to the Google OAuth client and to the Apple Services ID (domain and return URL) → `domains activate` → switch the app's Supabase URL with its session storage key pinned (nobody signed out), rebuild. About an hour of provider updates.
  - **What the free route did** (Alex, M3.1): the consent screen's app name was already "Pin'd"; its Authorised domain was `mxuajvlrkggrqpntekqt.supabase.co` and was changed to `pind.social`. **Result pending Google's cache — to be recorded here either way:** if that fixed the sentence, only the URL bar remains for this entry to fix; if it did not, neither the app name nor the authorised domain drives it, so nobody tries the free route again.
