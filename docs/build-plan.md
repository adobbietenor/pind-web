...
**Goal.** A person exists, can be reached, has a checked photo, and can delete themselves — on both platforms — before any of that is exposed to anyone.
...
- A2 you: DOB with the under-19 hard stop and year-only storage; gender (nonbinary offers women-only inclusion) into `people_private`; photo to the private bucket with a pending state.
...
- **Nothing waits on the check** (Alex, M3.1): a photo shows to the people who can see you **as soon as it uploads**. The check then follows the permissive rubric (decisions, "The photo check holds almost nothing now"): **a clear selfie, a cartoon and a group photo are each approved**; **only nudity, hate imagery or gore is rejected**, which removes the photo and leaves the profile visible without one; **a possible minor stays visible and goes to Alex's queue — and its owner is never told**. Every verdict appears in `moderation_log` as `ai:photo-check`.
...
- **M3.1's native-only walk, on M3.2b's build** (moved twice: M3.1 → M3.2 → M3.2b) (Alex, at the M3.1 merge — carried rather than holding the merge; CLAUDE.md, "Build when a milestone is ready to walk"). Everything else in M3.1 was walked on the web or proved by test, and the Done entry says which. On the phone, signed in to an account with a photo: **the photo picker** from the phone's own library (Change photo → Choose another → Save); **HEIC** — a photo straight from the iPhone camera is re-encoded or refused with its own sentence, never uploaded as something the check cannot read; **native Apple sign-in**; and **airplane mode with Wi-Fi off** (grey, not blue): open Change photo while online, then go offline, choose a photo and Save — the pick greys, the sentence reads "We could not upload your photo — Pin'd could not be reached…" with no hostname, and Remove photo brings the old photo back; then the Profile tab and Edit tags say "Pin'd could not be reached…" with **Try again**, never "Set up your profile" or an empty picker, and each loads once the network is back.
  ...
  - **And Google at A27 in the app** (added M3.2, with the shared identity step): the in-app browser link on the same anonymous user, and the merge when that Google account already has one. The web version is walked in M3.2 and checked by `check:link-path`.
...
- **A photo rejection fires live, once, end to end** (moved here from M3.1 by Alex: P87 already proves the database half, and a throwaway account exists here anyway). The photo leaves others' view within seconds, the profile stays visible without it, and `moderation_log` shows `ai:photo-check` / `photo_rejected`.
  - **The method — nothing harmful is ever stored anywhere of ours** (Alex, M3.1). The path has two halves, and each is proved by the tool that fits it:
    1. **Everything after the verdict, by a stubbed model.** A test drives `checkPhoto` with the model's answer stubbed to `reject`, against a harness person with a harmless placeholder photo, and asserts the verdict is recorded (`admin_record_photo_check`, `photo_checks`, `moderation_log` as `ai:photo-check` / `photo_rejected`) and the photo is gone from another person's view while its owner still has it (the P87 assertions, reached through the Worker's code rather than the RPC).
    2. **The model's own judgement, by the eval's dry mode.** `node --env-file=.dev.vars scripts/photo-check-eval.ts` without `--write` reads one rejectable image from local disk, sends it to the model and records nothing: the image never enters the bucket or the database, and the file is deleted afterwards.
    - **Not a corrupt image:** since M3.1 an image the model cannot see is a `failed` check, not a rejection, so it proves a different path. **Not a real image in the bucket:** the two halves above prove the same thing without storing one.
...
- A new Supabase project `pind-prod` on Pro (daily backups, no pausing), **in `ca-central-1` (Montreal) — a hard requirement** (Alex, M3.2: personal data on Canadian soil where reasonable, and the privacy policy says so; the region cannot be changed after the project is created, and a wrong one means redoing everything in `docs/configuration.md`), the same region as pind-staging, migrations applied by the CLI only, no seed data ever; secrets and Access for production; the Worker's production environment; Apple config for the production bundle ID; Resend on the live domain.
...
- **A custom auth domain** (`auth.pind.social`) — **not on staging; decided here** (Alex, M3.1). **It is the only fix for the Google sentence — the free route was tried and does not work** (below). Cost: **the $10/month custom-domain add-on per project, on top of Pro** (with Pro on both projects, about $70/month all in, as Alex priced it in M3.1). **It starts mattering the day strangers arrive from Reddit, not before**: the Google sheet then says "continue to mxuajvlrkggrqpntekqt.supabase.co", which reads like a phishing page. A project holds one custom domain. Doing it: the add-on on `pind-prod`, `supabase domains create` → a **DNS-only (grey cloud)** CNAME `auth` → the project host and the `_acme-challenge.auth` TXT it prints → `domains reverify` → add `https://auth.pind.social/auth/v1/callback` to the Google OAuth client and to the Apple Services ID (domain and return URL) → `domains activate` → switch the app's Supabase URL with its session storage key pinned (nobody signed out), rebuild. About an hour of provider updates.
  - **The free route was tried and does not work — do not spend an hour on it again** (Alex, M3.1). The consent screen's app name is "Pin'd" and its Authorised domain was changed from `mxuajvlrkggrqpntekqt.supabase.co` to `pind.social`, both saved. Tested well after Google's cache would have cleared, the account picker still says **"Sign in to continue to mxuajvlrkggrqpntekqt.supabase.co"**. So **neither the app name nor the authorised domain drives that sentence for this flow**: Google names the host the OAuth redirect goes to, which is the Supabase auth host until `auth.pind.social` replaces it. The Authorised domain stays `pind.social` regardless — it is the more correct value.
